...
public class BoardsService {
    ...
    public Page<BoardsResponseDto> getAllBoards(Pageable pageable) {
        return boardsRepository.findAll(pageable)
                .map(BoardsResponseDto::from);
    }

    //게시글 작성
    public BoardsResponseDto createBoard(BoardsCreateRequestDto boardsCreateRequestDto) {
        ...
        return BoardsResponseDto.from(board);
    }

    //게시글 열람
    @Transactional(readOnly = true)
    public BoardsResponseDto getBoard(Long boardId) {
    ...
    //게시글 수정
    @Transactional
    public BoardsResponseDto updateBoard(Long boardId, BoardsUpdateRequestDto boardsUpdateRequestDto) {
